...
public class ClanAlliesCommand {
    private final ExyliaClans plugin;

    public ClanAlliesCommand(ExyliaClans plugin) {
        this.plugin = plugin;
    }
        ...
        if (args.length == 1) {
            ...
            if (targetPlayer != null && targetPlayer.hasPlayedBefore()) {
                if (ClanUtils.isPlayerInClan(plugin, targetPlayer.getUniqueId().toString())) {
                    plugin.getMenuManager().openAlliesMenu(new InventoryPlayer(player), ClanUtils.getPlayerClanId(plugin, targetPlayer.getUniqueId().toString()));
                } else {
                ...
                plugin.getMenuManager().openAlliesMenu(new InventoryPlayer(player), clandId);
                return true;
            }
