...
public class ClanEnemiesCommand {
    private final ExyliaClans plugin;

    public ClanEnemiesCommand(ExyliaClans plugin) {
        this.plugin = plugin;
    }
        ...
        if (args.length == 1) {
            ...
            if (targetPlayer != null && targetPlayer.hasPlayedBefore()) {
                if (ClanUtils.isPlayerInClan(plugin, targetPlayer.getUniqueId().toString())) {
                    plugin.getMenuManager().openEnemiesMenu(new InventoryPlayer(player), ClanUtils.getPlayerClanId(plugin, targetPlayer.getUniqueId().toString()));
                } else {
                ...
                plugin.getMenuManager().openEnemiesMenu(new InventoryPlayer(player), clandId);
                return true;
            }
